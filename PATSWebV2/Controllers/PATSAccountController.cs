...
{
    public class PATSAccountController : Controller
    {
        ...
        public ActionResult Login(string returnUrl)
       {
        ...
        public async Task<ActionResult> Login(LoginInfo objLogin, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                else if (oUser.LoginFailures > PatsConstants.MaxLoginFailures)
                {
                ...
                {
                    if (string.IsNullOrEmpty(oUser.PasswordHash))
                    {
                        try
                        {
                            /*
                             * The App Pool of the web application is running under in IIS should be run under a user account which has appropriate privileges. 
                             * (Doesn't just have to be network service)
                             */
                            using (HostingEnvironment.Impersonate())
                            {
                                PrincipalContext adContext = new PrincipalContext(ContextType.Domain, PatsConstants.ActiveDirectory);
                ...
                if (authenticated == true) {
                    ...
                    RouteData.Values["action"].ToString(), HttpContext.Request.HttpMethod);
        ...
        {          
            try
            {
                ...
                {
                    var id = IdentityManagement.Data.SqlHelper.ExecuteQueryWithReturnValue(string.Format("SELECT UserID FROM dbo.[User] WHERE UserName='{0}'", HttpContext.Application["LoginUserName"]));
